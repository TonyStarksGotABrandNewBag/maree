...
The MITRE mapping is hand-curated and conservative. It covers five features (`imports_dangerous_api`, `identify_is_packed`, `Entropy ≥ 7.5`, `time_alignment_anomaly`, and DLL count extremes) and seven distinct technique IDs (T1055, T1106, T1027, T1027.002, T1140, T1070.006, plus combinations). Every (feature → technique) link is verified against the current MITRE ATT&CK matrix. Many real malware behaviors — registry persistence, lateral movement, command-and-control — are not mapped because the static PE features available to us do not unambiguously imply them.
