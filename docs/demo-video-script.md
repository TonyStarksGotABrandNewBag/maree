...
**Wyatt:** "Three layers of tests, per the rubric. **Unit tests** — fourteen files covering preprocessing (fit-on-train-only enforced by an explicit assertion), feature engineering, splits, models with consistent interfaces, the M.A.R.E.E. ensemble itself, the drift detector, and the triage layer. **Integration tests** — `test_app.py` exercises every endpoint of the Flask app: `/health`, `/predict` form route, `/api/predict` JSON route, `/upload` file route, all against an in-memory model fixture so they run in seconds. **Smoke tests** — `tests/test_smoke.py` plus the post-deploy `/health` poll in the CI workflow."
